Doorkeeper.configure do
  ...
  resource_owner_from_credentials do |routes|
    ...
    if user && user.failed_attempts < 5 && user.active_for_authentication? &&
      ...
      user
    elsif user && user.failed_attempts == 5
      raise Doorkeeper::Errors::DoorkeeperError, I18n.t('devise.failure.locked')
    elsif user && user.failed_attempts < 5
      raise Doorkeeper::Errors::DoorkeeperError, I18n.t('devise.failure.invalid', failed_attempts: 5 - user.failed_attempts)
    end
  end
